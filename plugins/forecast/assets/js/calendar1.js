(function() {
	...
	function _init() {
		...
		$('#calendar').on('mousedown', function (e) {
			// console.log(_calDom.fullCalendar('option','defaultView'));
				console.log(_calDom.fullCalendar('option','defaultView'));
			if (e.which == 3) {
				switch (_calDom.fullCalendar('option','defaultView')) {
					case 'month':
						// 
						$.when(
							_calDom.fullCalendar('changeView', 'agendaWeek'),
							_calDom.fullCalendar('option', 'defaultView','agendaWeek'),
							_calDom.fullCalendar('gotoDate', $(e.target).data('date'))
						).done(function(){
							console.log(_calDom.fullCalendar.options);
						});

						break;
				
					case 'agendaWeek':
						_calDom.fullCalendar('changeView', 'month');
						_calDom.fullCalendar('option', 'defaultView','month'),
						_calDom.fullCalendar('gotoDate', $(e.target).data('date'));
						break;
				} 
			}
	...
	const _calendar = {
		options: {
			...
			timeZone: 'UTC',
			header: {
				left: 'title myCustomButton myCustombutton',
		    ...
		    viewRender: (view, element) => {
	            ...
	            let start = moment(view.start._i).add(1, 'day').format('YYYY-MM-DD'),
	                ...
	                $('.fc-myCustomButton-button').attr('onclick', "");
			...
			eventRender: function(event, element) {
				let lengthRecipes = event.other.recipes == null || event.other.recipes == undefined ? 0 : event.other.recipes.length,
					icon = '';

				if(lengthRecipes > 0) {
					icon += '<a  data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Has ' + lengthRecipes + ' Recipe(s)!">';
					icon += '<i class="m-r-5 flaticon flaticon-clipboard pull-right"></i>';
					icon += '</a>';
				}

				if(event.other.notes != '') {
					icon += '<a data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Has Specific Notes">';
					icon += '<i class="m-r-5 flaticon flaticon-pencil-edit-button pull-right"></i>';
					icon += '</a>';
				}
